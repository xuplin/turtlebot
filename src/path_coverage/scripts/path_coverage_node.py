...
class MapDrive(MarkerVisualization):
	def __init__(self):
		...
		self.robot_width = rospy.get_param("~robot_width", 0.15)
		self.costmap_max_non_lethal = rospy.get_param("~costmap_max_non_lethal", 30)
		...
		#self.move_base.make_plan_add_unreachable_goal = False 
	...
	def rvizPointReceived(self, point):
		...
		if len(self.lClickPoints) > 2:
			...
			if dist_x_first_last < avg_x_dist/10.0 and dist_y_first_last < avg_y_dist/10.0:
				...
				self.visualize_area(points, close=True, show=True)
				self.lClickPoints = []
				return
	...
	def feedback_cb(self, feedback):
		...
		print("The distance between the current position and the goal is %f meters." % distance)
		
	def next_pos(self, x, y, angle,walk):
		...
		self.move_base.send_goal(goal, done_cb=self.done_cb,feedback_cb=self.feedback_cb)
	
		
		# rospy.sleep(5.0)
		#fini = self.move_base.wait_for_result(rospy.Duration(15))
		...
		# 	else:
		# 		rospy.logerr("The base failed moving to (%f, %f, %.0f)" % (x, y, angle*180/pi))
		#rospy.sleep(5.0)
		# print(self.move_base.wait_for_result())
		self.move_base.wait_for_result()
		print(self.move_base.wait_for_result(),self.move_base.get_state())
		rospy.sleep(5.0)
		# if self.move_base.get_state() == GoalStatus.SUCCEEDED:
		# 	rospy.loginfo("The base moved to (%f, %f, %.0f)" % (x, y, angle*180/pi))
		# else:
		# 	rospy.logerr("The base failed moving to (%f, %f, %.0f)" % (x, y, angle*180/pi))

	def done_cb(self, state, result:MoveBaseResult):
		if state == GoalStatus.ACTIVE:
			rospy.loginfo("Goal State: ACTIVE")
		elif state == GoalStatus.LOST:
			rospy.logerr("Goal State: LOST")
		elif state == GoalStatus.ABORTED:
			rospy.loginfo("Goal State: ABORTED")
		elif state == GoalStatus.SUCCEEDED:
			rospy.loginfo("Goal State: SUCCEEDED")
			rospy.loginfo("Result: "+ result._full_text)
		...
		for pos_last,pos_next in pairwise(path):
			...
			if abs(pos_diff[0]) < self.local_costmap_width/2.0 and abs(pos_diff[1]) < self.local_costmap_height/2.0:
				...
				if closest is None:
					continue
				pos_next = closest
			rospy.loginfo("next_pos: rotate")
			walk = False
			self.next_pos(pos_last[0], pos_last[1], angle,walk) # rotate in direction of next goal
			...
			# 	self.looprate.sleep()
			walk = True
			rospy.loginfo("next_pos: walk")
			self.next_pos(pos_next[0], pos_next[1], angle,walk)
