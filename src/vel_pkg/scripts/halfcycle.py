...
#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>#
#=============================#
def move_in_arc(radius,angle,velocity_publisher,log_file,segangle):
    vel_msg = Twist()
...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<#
#=============================#
def move_back_in_arc(radius, angle, velocity_publisher,log_file,segangle):
    vel_msg = Twist()
    #radius = 2.0  # 圓的半徑
    linear_speed = 0.2  # 線速度
    angular_speed = linear_speed / radius  # 角速度
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
#=============================#
def move_and_return(radius, angle, repetitions,segangle):
    ...
    with open(log_file_path, 'a') as log_file:
        for i in range(repetitions):
            ...
            #log_file.flush()
            move_in_arc(radius, angle, velocity_publisher, log_file,segangle)
            rospy.sleep(0.1)#休息
            move_back_in_arc(radius, angle, velocity_publisher, log_file,segangle)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--g', type=float, default=90,required=False, help='Angle to travel (in degrees)') #總角度
    parser.add_argument('--n', type=int, default=1, required=False, help='Number of repetitions') #來回次數
    parser.add_argument('--c', type=int, default=30, required=False, help='Number of sugment angle') #間隔角度
    args = parser.parse_args()
    try:
        
        move_and_return(args.r, args.g, args.n, args.c)
